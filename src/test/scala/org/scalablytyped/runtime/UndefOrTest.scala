...
class UndefOrTest {
  def some[A](v: A): UndefOr[A] = v
  def none[A]: UndefOr[A] = ().asInstanceOf[UndefOr[A]]

  // scala.scalajs.UndefOr[A]

  @Test def convert_A_to_js_UndefOr_A(): Unit = {
    val x: UndefOr[Int] = 42
  ...
  @Test def explicitly_convert_A_to_js_UndefOr_A(): Unit = {
    val x: UndefOr[Int] = some(42)
    assertFalse(x.isEmpty)
    assertEquals(42, x.get)

    val f: UndefOr[js.Function1[Int, Int]] = some((x: Int) => x + 1)
  ...
  @Test def flatMap(): Unit = {
    def f(x: Int): UndefOr[Int] = if (x > 0) x+3 else none
