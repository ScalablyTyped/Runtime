...
object AssertThrows {
  ...
  /** Backport implementation of Assert.assertThrows to be used until JUnit 4.13 is
    ...
    */
  private def assertThrowsBackport(expectedThrowable: Class[_ <: Throwable], runnable: ThrowingRunnable): Unit =
  ...
  /** Backport implementation of Assert.expectThrows to be used until JUnit 4.13 is
    *  released. See org.junit.Assert.scala in jUnitRuntime.
    */
  private def expectThrowsBackport[T <: Throwable](expectedThrowable: Class[T], runnable: ThrowingRunnable): T = {
    val result = {
      try {
        runnable.run()
        null.asInstanceOf[T]
