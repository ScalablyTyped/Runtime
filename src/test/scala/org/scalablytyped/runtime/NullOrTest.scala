...
class NullOrTest {
  def some[A](v: A): NullOr[A] = v
  def none[A]: NullOr[A] = null
  ...
  @Test def explicitly_convert_A_to_js_UndefOr_A(): Unit = {
    val x: NullOr[Int] = some(42)
    assertFalse(x.isEmpty)
    assertEquals(42, x.get)

    val f: NullOr[js.Function1[Int, Int]] = some((x: Int) => x + 1)
  ...
  @Test def flatMap(): Unit = {
    def f(x: Int): NullOr[Int] = if (x > 0) x+3 else none
    assertEquals(9, some(6).flatMap(f))
    assertNull(some(-6).flatMap(f))
    assertNull(none[Int].flatMap(f))
  }
  ...
  @Test def contains(): Unit = {
    ...
    assertFalse(some(null).contains(()))
  }
  ...
  @Test def collect(): Unit = {
    ...
    assertNull((some("hello") collect {
      case "notthis" => "ko"
    }))
    assertNull((none[String] collect {
      case "hello" => "ko"
    }))
  }
